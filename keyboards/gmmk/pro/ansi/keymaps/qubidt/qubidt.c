...
bool encoder_update_user(uint8_t index, bool clockwise) {
    ...
    uprintf("[encoder_update_user] %s\n", clockwise ? "clockwise" : "counter-clockwise");
#    ifdef NO_DEBUG
    print("[encoder_update_user] NO_DEBUG defined\n");
#    else
    print("[encoder_update_user] NO_DEBUG not defined\n");
#    endif  // NO_DEBUG
    dprint("[encoder_update_user] in debug mode\n");

    uprintf("[encoder_update_user] debug_enable: %s\n", debug_enable ? "yes" : "no");
    uprintf("[encoder_update_user] debug_matrix: %s\n", debug_matrix ? "yes" : "no");
    uprintf("[encoder_update_user] debug_keyboard: %s\n", debug_keyboard ? "yes" : "no");
    uprintf("[encoder_update_user] debug_mouse: %s\n", debug_mouse ? "yes" : "no");

    if (keyboard_report->mods & MOD_BIT(KC_LSFT)) {
        // If you are holding L shift, encoder changes layers

        dprintf("[encoder_update_user] KC_LSFT %s\n", clockwise ? "clockwise" : "counter-clockwise");
        ...
        // If you are holding R shift, Page up
        dprintf("[encoder_update_user] KC_RSFT %s\n", clockwise ? "clockwise" : "counter-clockwise");
        unregister_mods(MOD_BIT(KC_RSFT));
        register_code(clockwise ? KC_PGDN : KC_PGUP);
        register_mods(MOD_BIT(KC_RSFT));

    } else if (keyboard_report->mods & MOD_BIT(KC_LCTL)) {
        // if holding Left Ctrl, navigate next/prev word
        dprintf("[encoder_update_user] KC_LCTL %s\n", clockwise ? "clockwise" : "counter-clockwise");
        tap_code16(LCTL(clockwise ? KC_RGHT : KC_LEFT));

    } else if (keyboard_report->mods & MOD_BIT(KC_LALT)) {
        // if holding Left Alt, change media next/prev track
        dprintf("[encoder_update_user] KC_LALT %s\n", clockwise ? "clockwise" : "counter-clockwise");
        tap_code(clockwise ? KC_MEDIA_NEXT_TRACK : KC_MEDIA_PREV_TRACK);

    } else {
        switch (selected_layer) {
            case _FN1:
                dprintf("[encoder_update_user] _FN1 %s\n", clockwise ? "clockwise" : "counter-clockwise");
#    ifdef IDLE_TIMEOUT_ENABLE
...
#    endif
            ...
            default:
                // default behavior: increase volume
                dprintf("[encoder_update_user] default %s\n", clockwise ? "clockwise" : "counter-clockwise");
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    dprintf("[process_record_user] key (%d): %s\n", keycode, get_key_name(keycode));
    ...
    if (!record->event.pressed) {
        switch (keycode) {
            ...
            case RGB_TOD:
            case RGB_NITE:
                dprint("[process_record_user] unregistering\n");
